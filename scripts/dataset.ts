...
import { toCamelCase, toTitleCase } from "../src/lib/cases"
...
async function run() {
	...
	console.log("// Create a playwright context and browser")
	...
	console.log("// Close the playwright context and browser")
	async function teardown() {
		await context.close() // Close in reverse order
		await browser.close()
	}
	// Get package metadata
	console.log("// Get package metadata")
	...
	console.log("// Get names and anchors")
	...
	console.log("// Get SVG and JSX contents")
	...
	for (const icon of dataset.payload) {
		for (const info of [
			...
			{ page: p4, size: 16, variant: "solid"   },
		]) {

			console.log(icon[info.size][info.variant].href)
			const response = await info.page.goto(icon[info.size][info.variant].href)

			// If the page 404'd, continue
			if (response?.status() === 404) {
				console.log(`404: ${icon[info.size][info.variant].href}`)
				continue
			}

			//// await info.page.waitForSelector("svg")
			//// await info.page.waitForSelector("svg")
			//// await info.page.waitForSelector("svg")
	...
	console.log("// src/manifest@VERSION.ts")
	...
	console.log("// src/assets/heroicons@VERSION/index.ts")
	...
	for (const size of [16, 20, 24]) {
		...
		for (const variant of size !== 24 ? ["solid"] : ["outline", "solid"]) {
