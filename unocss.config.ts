...
function desugar(rawValue: undefined | string, { sign, px }: { sign?: string, px?: boolean } = {}): undefined | 0 | string {
	...
	const value = extractCSSVariables(convertSpaces(rawValue))
	if (value.startsWith("$")) {
		return value
...
const rules: Rule[] = [
	...
	[/^(-?)inset-x-(.+)$/,       ([_, sign, value]) => ({ "right":  desugar(value, { sign }), "left":   desugar(value, { sign }) })],
	[/^(-?)inset-y-(.+)$/,       ([_, sign, value]) => ({ "top":    desugar(value, { sign }), "bottom": desugar(value, { sign }) })],
	...
	[/^(-?)inset-tr-(.+)$/,      ([_, sign, value]) => ({ "top":    desugar(value, { sign }), "right":  desugar(value, { sign }) })],
	[/^(-?)inset-br-(.+)$/,      ([_, sign, value]) => ({ "right":  desugar(value, { sign }), "bottom": desugar(value, { sign }) })],
	[/^(-?)inset-bl-(.+)$/,      ([_, sign, value]) => ({ "bottom": desugar(value, { sign }), "left":   desugar(value, { sign }) })],
	[/^(-?)inset-tl-(.+)$/,      ([_, sign, value]) => ({ "top":    desugar(value, { sign }), "left":   desugar(value, { sign }) })],
	...
	[/^flex-align-self-(.+)$/,   ([_, value]) => ({ "align-self":   desugar(value, { px: false }) })],
	...
	//// [/^flex-(.+)$/,              ([_, value]) => ({ "flex":         desugar(value) })],
	...
	// Shorthand
	//// [/^flex-(.+)$/,              ([_, value]) => ({ "justify-content": desugar(value), "align-items": desugar(value) })],
	...
	[/^grid-(\d+)$/,             ([_, x]) => ({ "display": "grid", "grid-template":         x === "1" ? "1fr" : `repeat(${x}, 1fr)` })], // TODO
	[/^grid-rows-(\d+)$/,        ([_, y]) => ({ "display": "grid", "grid-template-rows":    y === "1" ? "1fr" : `repeat(${y}, 1fr)` })], // TODO
	[/^grid-cols-(\d+)$/,        ([_, x]) => ({ "display": "grid", "grid-template-columns": x === "1" ? "1fr" : `repeat(${x}, 1fr)` })], // TODO
